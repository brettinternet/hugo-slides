...
(function() {
  ...
  function decreaseFontSize() {
    const notesValueStyles = window.getComputedStyle(notesValue);
    const currentFontSize = notesValueStyles.getPropertyValue("font-size");
    const value = parseInt(currentFontSize, 10);
    const unitsMatch = currentFontSize.match(/\D/);
    const units = currentFontSize.slice(unitsMatch.index);
    const newValue = value - fontStep;
    notesValue.style.fontSize =
      (newValue <= fontSizeFloor ? value : newValue) + units;
  }
